...
GeneticRpnSolver::GeneticRpnSolver():population(10),maxIterations(50),
...
void GeneticRpnSolver::init(int population, int maxIterations, float maxTime, float requiredVolume, float mutationProb, int selectionCount, int crossingoverCount, int repairAttempts, bool rotateBoxes, bool compressBoxes)
{
...
QVector<BoxInfo> GeneticRpnSolver::solve(const QVector<Box> &boxes, const Box &bounds)
{
    log.clear();
    ...
    if(bestVolume<10e8)
    {
        ...
        log+="Решение получено генетическим алгоритмом с использование обратной польской нотации для представления расположения блоков\n";
        ...
        log+="Уплотнение: "+(this->compressBoxes?"используется\n":"не используется\n");
...
{
    //Генерация дерева расположения ящиков на основе обратной польской нотации
    ...
    {
        //Выбрать два случайных узла и применить к ним случайную операцию
...
void reduction(Chromosome& c,const float wProb,const float hProb,const bool rotateBoxes)
{
    for(int j=0;j<c.genes.length();j++)
    {
        if(c.genes[j]<0)
        {
           //Выбрать ориентацию случайным образом на основе пропорций ящика
...
void recompute(QList<AABB>* aabbList,float minx,float maxx,float miny,float maxy,float minz,float maxz)
{
    int listSize=aabbList->count();
    for(int j=0;j<listSize;j++)
    {
        AABB t_aabb=aabbList->operator[](j);
...
void reduceAABB(QList<AABB>* aabbList)
{
    //check for inscribed spaces
    for(int l=aabbList->length()-1;l>=0;l--)
    {
        AABB c1=aabbList->operator [](l);
...
Chromosome generateGreed2(const QVector<Box>& boxes,const Box& bounds,bool rotateBoxes)
{
    ...
    while(macroblocks.length()>1)
    {
        ...
        int s=0;
        int op=-1;
        int fitnessCriteria=Random::random(3);
